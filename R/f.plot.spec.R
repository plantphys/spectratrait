...
##' @param CI Desired confidence interval for the spectra plot. Default is: 95; can be changed dynamically
##' 
...
f.plot.spec <- function(
  ...
  CI = 95             ## confidence interval as percentage, default 95% ; can be prescribed  
){
  ...
  CI_l=0+((100-95)/200) # lower bound for the confidence interval  
  CI_u=1-((100-95)/200) # upper bound for the confidence interval 
  
  spectra_quantiles <- apply(Z,2,quantile,na.rm=T,probs=c(0,CI_l,CI_u,1)) 
  
  plot(x=NULL,y=NULL,ylim=c(0,100),xlim=xlim,xlab="Wavelength (nm)",
       ylab=paste0(type," (%)"),main=plot_label)
    
  polygon(c(wv ,rev(wv)),c(spectra_quantiles[3,]*100, rev(spectra_quantiles[2,]*100)),
          col="#99CC99",border=NA)
      
       
  ci_text=paste0(CI,"% CI")          
  lines(wv,mean_spec*100,lwd=2, lty=1, col="black")
  lines(wv,spectra_quantiles[1,]*100, lty=3, col="grey40")
  lines(wv,spectra_quantiles[4,]*100, lty=3, col="grey40")
