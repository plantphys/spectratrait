...
#
#
#    --- Last updated:  12.03.2019 By Shawn P. Serbin <sserbin@bnl.gov>
####################################################################################################
...
list.of.packages <- c("devtools","readr","scales","plotrix","httr","pls","dplyr",
                      "reshape2","ggplot2")  # packages needed for script
...
trait_data2 <- melt(data = trait_data, id.vars = "USDA_Species_Code", measure.vars = c("LMA_gDW_m2",
                                                                                       "Nmass_g_g",
                                                                                       "N_area_g_m2",
                                                                                       "CN_Ratio"))
head(trait_data2)
...
ggplot(trait_data2, aes(x=USDA_Species_Code, y=note, fill=treatment)) + 
  geom_boxplot()


p2 <- ggplot(trait_data2, aes(x=USDA_Species_Code, y=value)) + 
  geom_boxplot() +
