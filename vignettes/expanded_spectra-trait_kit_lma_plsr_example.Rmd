...
params:
  date: !r Sys.Date()
title: Spectra-trait PLSR example using leaf-level spectra and leaf mass per area (LMA) data from more than 40 species grassland species comprising both herbs and graminoids. 
---
...
Jackknife_coef <- f.coef.valid(plsr.out = jk.plsr.out, 
                               data_plsr = cal.plsr.data, ncomp = nComps)
...
jk_val_scatterplot <- ggplot(val.plsr.output, aes(x=PLSR_Predicted, 
                                                  y=get(inVar))) + 
  theme_bw()+ geom_errorbar(aes(xmin = LPI,xmax = UPI),color='grey', 
                            width=0.2) + 
  geom_errorbar(aes(xmin = LCI,xmax = UCI),color='blue',width=0.2) + 
  geom_point(size=1.3)  + 
