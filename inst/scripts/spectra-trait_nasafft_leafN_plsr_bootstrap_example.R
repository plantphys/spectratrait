...
#   Serbin et al., 2014 Spectroscopic determination of leaf morphological and biochemical traits \ 
#   for northern temperate and boreal tree species
#   DOI: https://doi.org/10.1890/13-2110.1
#   https://ecosis.org/package/nasa-fft-project-leaf-reflectance-morphology-and-biochemistry-for-northern-temperate-forests
#
#    Notes:
#    * The author notes the code is not the most elegant or clean, but is functional 
#    * Questions, comments, or concerns can be sent to shawn.p.serbin@nasa.gov
...
ecosis_id <- "fea0294d-2d39-4576-aab5-7200fdc87068"
...
sample_info2 <- sample_info %>%
  select(Species_Code=`USDA Symbol`,Nmass_g_g=`Nmass_perc`)
head(sample_info2)
...
#split_seed <- 1245565
split_seed <- 85715
split_data <- spectratrait::create_data_split(dataset=plsr_data, approach=method, split_seed=split_seed, 
...
cal.plsr.data <- data.frame(cal.plsr.data[, which(names(cal.plsr.data) %notin% paste0("Wave_",wv))],
                            Spectra=I(cal_spec))
head(cal.plsr.data)[1:2]

val_spec <- as.matrix(val.plsr.data[, which(names(val.plsr.data) %in% paste0("Wave_",wv))])
val.plsr.data <- data.frame(val.plsr.data[, which(names(val.plsr.data) %notin% paste0("Wave_",wv))],
                            Spectra=I(val_spec))
head(val.plsr.data)[1:2]
#--------------------------------------------------------------------------------------------------#
